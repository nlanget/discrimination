...
def make_SDS_data_links(datadir,outdir):
  ...
  #all_sta.sort()
  all_sta = ['/media/disk/CLASSIFICATION/mllr/grad']

  filedict = {}
  for sta in all_sta:
    all_files = glob.glob(os.path.join(sta,'ID*'))
    all_files.sort()
  ...
  for dirid,filelist in filedict.iteritems():
    ...
    sta = dirid.split('.')[1]
    cha = dirid.split('.')[2]
    dirname=os.path.join(out_dir,net,sta,"%s.D/GRAD"%cha)
    try:
      os.makedirs(dirname)
...
if __name__ == '__main__':
  ...
  make_SDS_data_links('/media/disk/CLASSIFICATION','../data/Ijen')
